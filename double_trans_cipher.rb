  ...
  def self.encrypt(document, key)
    ...
    matrix_columns_shuffle.each_with_index do |row, index| 
      @col_order.each_with_index do |e, i| 
        matrix_columns_shuffle[index][i] = matrix_rows_shuffle[index][e]
      end
    end
    matrix_columns_shuffle.join()
  end

  def self.decrypt(ciphertext, key)
    # TODO: FILL THIS IN!
    #ciphertext = "tkcaatatnadw"
    ...
    @row_order.each_with_index{ |e, i| matrix_rows_ordered[e] = matrix_columns_ordered[i] }
    matrix_rows_ordered.join().to_s.force_encoding('utf-8')
  end
end
